# Fog of War: A Bayesian Survival Game
...
Fog of War is an interactive, turn-based dungeon game where every core mechanic is built on a concept from CS109. The player navigates a 16×16 grid dungeon shrouded in fog, avoiding hidden monsters while making their way to the exit. The twist: you can't see the monsters directly. Instead, each turn your sonar sensor gives you a **noisy distance reading** to the nearest monster, and you must use **Bayesian inference** to build and maintain a probabilistic belief about where the monsters are.

The game is not just a visualization — the probability is the gameplay. Players who understand and use the Bayesian heatmap, expected value arrows, and information gain panel will measurably outperform players who move randomly. The math isn't window dressing; it's your survival tool.
...
### Setup
- Open the game in a browser via a local server (e.g., `python3 -m http.server 8080`)
- Click **Tutorial** for a guided walkthrough of the Bayesian concepts, or **Play** to jump straight in
...
### 2. Gaussian Distribution (Lecture 9)
...
### 3. Conditional Probability / Markov Chains (Lecture 2)
...
### 5. Shannon Entropy / Information Theory (Lecture 17)
...
### 6. Beta Distribution (Lecture 14)
...
### 9. Prediction via Convolution (Gaussian Motion Model)
...
| File | Purpose |
|------|---------|
| `index.html` | Page layout, module initialization, game loop |
| `styles.css` | All styling, animations, tutorial effects |
| `math.js` | CS109 math library — Gaussian, entropy, Beta PDF, bootstrap, convolution |
| `audio.js` | Sound effects + Markov chain music engine with transition matrices |
| `gamestate.js` | Game logic, dungeon generation, Bayesian inference engine, particles |
| `renderer.js` | Canvas rendering, smooth player interpolation, fog gradients |
| `panels.js` | All side panel visualizations (entropy, Beta, EV, info gain, etc.) |
| `tutorial.js` | 8-step guided walkthrough teaching Bayesian concepts |

All probability math is implemented from first principles — no external probability libraries. The only dependencies are the browser's Canvas API and Web Audio API.

---

## Technical Notes

- **Gaussian random numbers** are generated via the Box-Muller transform
- **Beta PDF** uses the Lanczos approximation for the log-gamma function
- **Convolution** is done naively (nested loops) — fast enough on a 16×16 grid (256 cells × 25 kernel entries = 6400 operations)
- **Belief** is stored as a `Float64Array(256)` for performance, indexed as `row * 16 + col`
- **Dungeon generation** uses a biased random walk to guarantee a path from start to exit, then scatters walls and monsters with distance constraints
- **Music tempo** scales with tension: 400ms between notes at maximum tension, 700ms when calm

---

## LLM Disclosure

This project was developed with assistance from Claude (Anthropic). The conceptual design, feature selection, and CS109 concept integration were collaborative. All probability implementations were written from scratch without using probability libraries.
